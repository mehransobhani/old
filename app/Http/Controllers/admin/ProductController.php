...
class ProductController extends Controller
{
	public function index(Request $request)
	{
		...
		return View('product.index', ['product' => $product, 'data' => $request->all(), 'cat_list' => $cat_list]);
	}
	...
	public function store(ProductRequest $request)
	{
		...
		$Product->limited        = ($request->has('limited')) ? $request->get('limited') : 0;
		...
		$color_inventory = $request->get('color_inventory');
			...
			foreach ($color_name as $key => $value)
			{
				if (!empty($value) && !empty($color_code[$key]))
				{
					DB::table('color_product')->insert([
					    ...
					    'price' => $color_price[$key],
					    'inventory' => $color_inventory[$key]
					    ]);
		...
		if (is_array($color_name))
		{
			foreach ($color_name as $key => $value)
			{
				if ($key > 0)
				{
					if (!empty($value))
					{
						DB::table('color_product')->where('id', $key)->update([
						    'color_name' => $value, 
						    'color_code' => $color_code[$key],
						    'price' => $color_price[$key],
						    'inventory' => $color_inventory[$key]
