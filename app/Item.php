...
class Item extends Model
{
	...
	public function isItemValueFilled($items, $product_id)
	{
		$itemId = array();
		...
		$filledItemValue = DB::table('item_product')->where(['product_id' => $product_id])->whereIn('item_id', $itemId)->where('value', '!=', null)->count();

		if ($filledItemValue > 0)
			return true;
