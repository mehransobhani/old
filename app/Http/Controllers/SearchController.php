...
class SearchController extends Controller
{
    ...
    public function productSearchTerm(Request $request)
    {
        $search = $request->term;

        //$products = Product::with('get_img')->where('title', 'like', "%{$search}%")->paginate(5);
        if ($request->ajax()) {
            $products = Product::with('get_img')->where('title', 'like', "%{$search}%")->get();
            ...
            foreach ($products as $product) {
                ...
                foreach ($product->categories as $category) {
                    $productCategories .= '<span>' . $category->cat_name . '</span>';

                    $innerLoopIndex++;

                    if ($innerLoopIndex > 0 && $innerLoopIndex < sizeof($product->categories))
                        $productCategories .= '<span> - </span>';


                }
                $new_row['title'] = $product->title;
                $new_row['category'] = $productCategories;
                $new_row['image'] = asset('upload/' . $product->get_img->url);
                $new_row['url'] = route('site.showProductDetails', [$product->code_url, $product->title_url]);
            ...
            if (count($products) > 5) {
                $row_set[] = array(
                    'show_more' => true,
                    'url' => route('searchProducts', ['term' => $request->term]),
                );
            }
            if(isset($row_set))
            ...
            return view('front-end.search_result', compact('search', 'products'));
        }
    }
    ...
    public function ajax_search(Request $request)
    {
        if ($request->ajax()) {
            ...
            return view('front-end.ajax_product_category', [
                'data' => $data,
                'product' => $data['product'],
                'cat_url' => $cat_url,
                'total_product' => $data['total_product']
    ...
    public function show_cat1($cat1, $cat2)
    {
        ...
        $filter = Filter::get_search_filter($category1->id, $category2->id);
        //dd($data);
        ...
        return view('front-end.product_category_level_two_lara_digi', [
            'data' => $data,
            'product' => $data['product'],
            'filter_id' => '',
            'category1' => $category1,
        ...
        if ($this->view == '') {
            ...
            //return View('site.product_cat2', ['data' => $data, 'category1' => $category1, 'price' => $price, 'cat_url' => $cat_url, 'cat_list' => $cat_list, 'category2' => $category2]);
            return view('front-end.product_category_level_two', ['data' => $data, 'category1' => $category1, 'price' => $price, 'cat_url' => $cat_url, 'cat_list' => $cat_list, 'category2' => $category2, 'filter' => $filter]);
        } else {
    ...
    public function cat1($cat)
    {
        ...
        return view('front-end.product_category_level_one', [
            'data' => $data,
            'filter_id' => '',
            'category1' => $category,
            'price' => $price,
