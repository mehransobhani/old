<form method="post" action="{{ url('admin/service/set_price/?product_id='.$product_id) }}">
    ...
    @foreach($product_color as $key=>$value)
        ...
        <div class="form-group">
            ...
            <input type="text" name="color[<?= $value->id ?>]" value="{{ $price }}" class="form-control"
                   style="text-align:left" placeholder="هزینه بر حسب تومان و به صورت عدد وارد شود">
        </div>
    ...
    @if(sizeof($product_color)==0)
        ...
        <div class="form-group">
            ...
            <input type="text" name="color[0]" value="{{$service_price?$service_price[0]:''}}" class="form-control"
                   style="text-align:left"
                   placeholder="هزینه بر حسب تومان و به صورت عدد وارد شود">
        </div>
    @endif
