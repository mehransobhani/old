                        ...
                        <div class="product-view-area">
                            <div class="product-big-image col-xs-12 col-sm-5 col-lg-5 col-md-5">
                                ...
                                @if($product->discounts>0)
                                    <div class="icon-sale-label sale-left">@lang('front-end.product.sale')</div>
                                @endif
                                ...
                                <div class="price-box">
                                    @if($product->discounts>0)
                                        <p class="special-price"><span class="price-label">Special Price</span> <span
                                                    class="price"> {{number_format($product->price-$product->discounts)}} @lang('front-end.product.price_label')</span>
                                        </p>
                                        <p class="old-price"><span class="price-label">Regular Price:</span> <span
                                                    class="price">  {{number_format($product->price)}} </span></p>
                                    @elseif($product->price>0)
                                        <p class="special-price"><span class="price-label">Special Price</span> <span
                                                    class="price"> {{number_format($product->price)}} @lang('front-end.product.price_label')</span>
                                        </p>
                                    @else
                                        <p class="special-price"><span class="price-label">Special Price</span> <span
                                                    class="price">بدون قیمت</span>
                                        </p>
                                    @endif
                                </div>
                                <div class="ratings">
                                    ...
                                    <p class="availability in-stock pull-left">@lang('front-end.product.availability')
                                        <span>@if($product->product_status==1) @lang('front-end.product.available') @endif</span>
                                    </p>
                                </div>
                                        ...
                                        <div class="color">
                                            <ul>
                                                @foreach($product->get_colors as $color)
                                                    <li><a href="#" title="{{$color->color_name}}"
                                                           onclick="set_color({{$color->id}})"
                                                           id="color_{{$color->id}}"
                                                           style="background: {{'#'.$color->color_code}}"></a></li>
                                                @endforeach
                                            </ul>
                                        </div>
                                    </div>
...
@section('bottom_script')
    ...
    <script>
        function set_color(color_id) {
            if (color_id) {
                var check_color = '#fff';
                var old_color = document.getElementById('color_id').value;

                document.getElementById('color_id').value = color_id;
                var color_element = document.getElementById('color_' + color_id);
                if (lightOrDark(color_element.style.backgroundColor) == 'light')
                    check_color = '#000';
                color_element.innerHTML = "<i class='fa fa-check' id='check_color_" + color_id + "' style='color:" + check_color + ";padding-right: 3px'></i>";
                if (old_color != '0') {
                    document.getElementById("check_color_" + old_color).remove();
                }
            }
        }
